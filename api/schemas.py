...
class SensorReading(BaseModel):
    ...
    sensor_7:  float = Field(..., description="Total pressure at HPC outlet (P30)")
    sensor_8:  float = Field(..., description="Physical fan speed (Nf)")
    sensor_9:  float = Field(..., description="Physical core speed (Nc)")
    sensor_11: float = Field(..., description="Static pressure at HPC outlet (Ps30)")
    sensor_12: float = Field(..., description="Fuel-flow to Ps30 ratio (phi)")
    sensor_13: float = Field(..., description="Corrected fan speed (NRf)")
    sensor_14: float = Field(..., description="Corrected core speed (NRc)")
    sensor_15: float = Field(default=8.0,   description="Bypass ratio (BPR)")
    sensor_17: float = Field(default=390.0, description="Bleed enthalpy (htBleed)")
    sensor_20: float = Field(default=39.0,  description="HPT coolant bleed (W31)")
    sensor_21: float = Field(default=23.0,  description="LPT coolant bleed (W32)")
